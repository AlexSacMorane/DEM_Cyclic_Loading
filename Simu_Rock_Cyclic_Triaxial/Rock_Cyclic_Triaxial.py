...
# command yade -j n_processors -n -x Rock_Cyclic_Triaxial.py
...
def checkUnbalanced_load_deviatoric_ic():
    ...
    global L_unbalanced_cycle, L_confinement_x_cycle, L_confinement_y_cycle, L_confinement_z_cycle, L_count_bond_cycle, L_margin_bond_cycle, L_q_cycle, L_eps_dev_cycle
    L_unbalanced_cycle = []
...
def checkUnbalanced():
    ...
    L_q_cycle.append(1/2*(O.forces.f(plate_x_max.id)[0]-O.forces.f(plate_x_min.id)[0])/2/((plate_y_max.state.pos[1]-plate_y_min.state.pos[1])*(plate_z_max.state.pos[2]-plate_z_min.state.pos[2]))+\
                     1/2*(O.forces.f(plate_y_max.id)[1]-O.forces.f(plate_y_min.id)[1])/2/((plate_x_max.state.pos[0]-plate_x_min.state.pos[0])*(plate_z_max.state.pos[2]-plate_z_min.state.pos[2]))-\
                     (O.forces.f(plate_z_max.id)[2]-O.forces.f(plate_z_min.id)[2])/2/((plate_x_max.state.pos[0]-plate_x_min.state.pos[0])*(plate_y_max.state.pos[1]-plate_y_min.state.pos[1])))
    # track deviatoric strain
    eps_x = ((plate_x_max.state.pos[0]-plate_x_min.state.pos[0])-(plate_x_max.state.refPos[0]-plate_x_min.state.refPos[0]))/(plate_x_max.state.refPos[0]-plate_x_min.state.refPos[0])
    eps_y = ((plate_y_max.state.pos[1]-plate_y_min.state.pos[1])-(plate_y_max.state.refPos[1]-plate_y_min.state.refPos[1]))/(plate_y_max.state.refPos[1]-plate_y_min.state.refPos[1])
    eps_z = ((plate_z_max.state.pos[2]-plate_z_min.state.pos[2])-(plate_z_max.state.refPos[2]-plate_z_min.state.refPos[2]))/(plate_z_max.state.refPos[2]-plate_z_min.state.refPos[2])
    L_eps_dev_cycle.append(100*2/3*(0.5*(eps_z-eps_x)+0.5*(eps_z-eps_y)))
    ...
    # eps_v - p
    ax4.plot(L_eps_dev_cycle, L_q_cycle)
    ax4.set_title(r'q (Pa) - $\epsilon_{dev}$ (%)')
    # close
...
def stopLoad():
    ...
    # save simulation
    os.mkdir('../Data_Rock_Cyclic_Triaxial/'+O.tags['d.id'])
    shutil.copytree('data','../Data_Rock_Cyclic_Triaxial/'+O.tags['d.id']+'/data')
    shutil.copytree('plot','../Data_Rock_Cyclic_Triaxial/'+O.tags['d.id']+'/plot')
    shutil.copy('Rock_Cyclic_Triaxial.py','../Data_Rock_Cyclic_Triaxial/'+O.tags['d.id']+'/Rock_Cyclic_Triaxial.py')
    shutil.move(O.tags['d.id']+'_report.txt','../Data_Rock_Cyclic_Triaxial/'+O.tags['d.id']+'/'+O.tags['d.id']+'_report.txt')
...
def saveData():
    ...
    if len(lines) >= 3:
        for i in range(len(data)):
            ...
            L_sigma_dev.append(1/2*L_sigma_x[-1]+1/2*L_sigma_y[-1]-L_sigma_z[-1])
            ...
            L_dev_strain.append(abs(2/3*(0.5*(L_strain_z[-1]-L_strain_x[-1])+0.5*(L_strain_z[-1]-L_strain_y[-1]))))

        # plot
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2,2, figsize=(16,9),num=1)

        ax1.plot(L_dev_strain, L_sigma_dev)
        ax1.set_title(r'q vs. $\epsilon_{dev}$')
        ax1.set_xlabel(r'$\epsilon_{dev}$ (%)')
        ax1.set_ylabel(r'Deviatoric stress (Pa)')
